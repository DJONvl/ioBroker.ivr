...
module.exports = function (grunt) {
    ...
    var author    = grunt.option('author') || 'Vlad';
    ...
    if (process.argv[2] == 'rename') {
        ...
        if (!newname) {
            console.log('Please write the new ivr name, like: "grunt rename --name=mywidgetset" --author="Author Name"');
            process.exit();
        }
        ...
        if (fs.existsSync(__dirname + '/admin/ivr.png')) {
            fs.renameSync(__dirname + '/admin/ivr.png',              __dirname + '/admin/' + newname + '.png');
        }
        if (fs.existsSync(__dirname + '/widgets/ivr.html')) {
            fs.renameSync(__dirname + '/widgets/ivr.html',           __dirname + '/widgets/' + newname + '.html');
        }
        if (fs.existsSync(__dirname + '/widgets/ivr/js/ivr.js')) {
            fs.renameSync(__dirname + '/widgets/ivr/js/ivr.js', __dirname + '/widgets/ivr/js/' + newname + '.js');
        }
        if (fs.existsSync(__dirname + '/widgets/ivr')) {
            fs.renameSync(__dirname + '/widgets/ivr',                __dirname + '/widgets/' + newname);
        }
    }
            ...
            name: {
                options: {
                    patterns: [
                        ...
                        {
                            match: /Ivr/g,
                            replacement: newname ? (newname[0].toUpperCase() + newname.substring(1)) : 'Ivr'
                        },
                        {
                            match: /Vlad/g,
                            replacement: author
